...
const Page = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "block" | "not_block">("all");
  ...
  const fetchCustomers = useCallback(
    async (page: number) => {
      ...
      try {
        const response = await api.get("/api/users/customer-list", {
          params: {
            page,
        ...
        setMessage(axiosError.response?.data?.message ?? "Failed to fetch customers");
      } finally {
        setLoading(false);
      }
    },
  ...
  const updateStatus = async (userId: number, status: "block" | "not_block") => {
    ...
    try {
      const response = await api.patch("/api/users/customer-list", {
        userId,
        status,
      });
      setMessage(response.data?.message ?? "Customer status updated successfully");
      await fetchCustomers(pagination.page);
    } catch (error: unknown) {
      const axiosError = error as AxiosError<{ message?: string }>;
      setMessage(axiosError.response?.data?.message ?? "Failed to update customer status");
    } finally {
      setUpdatingId(null);
    }
  };
        ...
        <div className="rounded-2xl border border-[#4F6D7A]/30 bg-[linear-gradient(145deg,#ffffff_0%,#f1f1f4_100%)] p-5 shadow-[0_10px_30px_rgba(89,89,89,0.12)] md:p-6">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <select
              value={statusFilter}
              onChange={(event) =>
                setStatusFilter(event.target.value as "all" | "block" | "not_block")
              }
              className="h-10 rounded-md border border-[#C0D6DF]/70 bg-white px-3 py-2 text-sm text-[#1F3B4D] outline-none ring-[#4F6D7A]/30 focus:ring-2"
            >
              <option value="all">All Status</option>
              <option value="not_block">Not Block</option>
              <option value="block">Block</option>
            </select>
            <button
              type="button"
              onClick={() => void fetchCustomers(1)}
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          onClick={() =>
                            updateStatus(
                              customer.id,
                              customer.status === "not_block" ? "block" : "not_block",
                            )
                          }
